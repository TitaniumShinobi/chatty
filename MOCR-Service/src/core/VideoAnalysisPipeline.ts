...
import { MOCRVisualProcessor, MOCRVisualResult, MOCROptions } from './MOCRVisualProcessor';
...
export class VideoAnalysisPipeline {
  ...
  /**
   * Analyze video from file path (for large files - avoids loading into memory)
   */
  static async analyzeVideoFromPath(
    videoPath: string,
    options: VideoAnalysisOptions = {}
  ): Promise<VideoAnalysisResult> {
    return this.analyzeVideoInternal(videoPath, options);
  }
  ...
  static async analyzeVideo(
    ...
    try {
      return await this.analyzeVideoInternal(tempVideoPath, options);
    } finally {
      this.cleanupTempFile(tempVideoPath);
    }
  }

  private static async analyzeVideoInternal(
    videoPath: string,
    options: VideoAnalysisOptions = {}
  ): Promise<VideoAnalysisResult> {
    ...
    try {
      ...
      const videoMetadata = await VideoFrameExtractor.getVideoMetadata(videoPath);
      ...
      // Run MOCR and ASR in parallel (path-based - no memory load)
      const [mocrResult, asrResult] = await Promise.allSettled([
        mergedOptions.enableMOCR ? MOCRVisualProcessor.processVideoFromPath(videoPath, mergedOptions.mocr) : Promise.resolve(null),
        mergedOptions.enableASR ? ASRService.transcribeVideoFromPath(videoPath, mergedOptions.asr) : Promise.resolve(null)
      ]);
      ...
      return {
        success: true,
        videoMetadata,
        mocrResult: mocr ?? undefined,
        asrResult: asr ?? undefined,
        synchronizedContent,
        contentSummary,
        processingTime
      };
  ...
  private static generateContentSummary(
    videoMetadata: VideoMetadata,
    mocrResult: MOCRVisualResult | null,
