...
export class VideoFrameExtractor {
  private static readonly DEFAULT_MAX_FRAMES = 1000;
  private static readonly DEFAULT_INTERVAL = 10; // 10 seconds (feature-length: ~1000 frames for 3hr film)
  ...
  static async extractFrames(
    ...
    return new Promise((resolve, reject) => {
        ...
        .on('end', async () => {
          try {
            ...
            for (const frameFile of frameFiles) {
              ...
              // Get dimensions from image header (no native deps)
              const dims = imageSize(imageData) || {};
      ...
      if (thumbnailOnly) {
        ...
        // Extract frames at intervals (-vf is output filter, must be in outputOptions)
        command
          .seekInput(startTime)
          .outputOptions([`-vf`, `fps=1/${interval}`, `-q:v`, quality.toString()])
