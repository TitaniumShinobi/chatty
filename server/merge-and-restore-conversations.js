...
async function createMergeRestoreScript() {
  console.log('🔄 MERGE AND RESTORE CONVERSATIONS SCRIPT');
  console.log('====================================');
  console.log(`Timestamp: ${new Date().toISOString()}\n`);

  const mergeRestoreScript = `
// MERGE AND RESTORE CONVERSATIONS SCRIPT
// This script merges localStorage conversations with backend data and prevents overwrites
...
`;
  ...
  // Write the merge restore script
  fs.writeFileSync('merge-restore-conversations-browser.js', mergeRestoreScript);
  console.log('📄 Merge and restore script created: merge-restore-conversations-browser.js');
  
  console.log('\n📋 TO MERGE AND RESTORE YOUR CONVERSATIONS:');
  console.log('1. Copy the script from merge-restore-conversations-browser.js');
  console.log('2. Open your browser console (F12)');
  console.log('3. Paste and run the script');
  console.log('4. This will merge local and backend conversations and save them properly');
  
  console.log('\n🎯 WHAT THIS SCRIPT DOES:');
  console.log('- Extracts all conversations from localStorage');
  console.log('- Fetches current backend conversations');
  console.log('- Merges them without duplicates');
  console.log('- Saves merged conversations to backend');
  console.log('- Updates localStorage with the complete set');
  console.log('- Refreshes the UI to show all conversations');
}

// Run the script creation
createMergeRestoreScript();
