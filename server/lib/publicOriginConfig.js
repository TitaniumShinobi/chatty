import { resolveDoorContract } from "./chattyVvaultDoorConfig.js";
...
function resolveDoorPublicOrigin(env = process.env) {
  return resolveDoorContract(env).chattyPublicOrigin || null;
}

function resolveDoorCallbackBase(env = process.env) {
  return resolveDoorContract(env).chattyApiOrigin || null;
}
...
export function resolveConfiguredCanonicalDomain(env = process.env) {
  ...
  const doorOrigin = parseHttpUrl(resolveDoorPublicOrigin(env));
  return doorOrigin?.hostname || null;
}
