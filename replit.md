...
- **Provider Fallback Telemetry:** Structured observability for the provider fallback chain. `tryProvider()` helper with 12s AbortController timeout and 1 retry (on timeout/5xx only). Captures per-attempt data (provider, status, error_code, duration_ms, retry count). `provider_trace` object exposed in API responses when `SHOW_DEV_INFO=true`. Concise one-line `📡 [ProviderTrace]` log per request. Frontend dev-info panel shows color-coded provider attempt history with checkmark/X indicators.
