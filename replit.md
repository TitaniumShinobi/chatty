...
- **February 1, 2026:** Made FXShinobi the sole broker client. Chatty no longer requires or stores OANDA credentials; all broker data (chart symbol/timeframe, account balance, P&L, environment) comes from FXShinobi's `/api/snapshot` and `/api/status` endpoints via `/api/fxshinobi/*` proxies. ChartProvider now accepts dynamic `timeframe` prop with TradingView interval mapping. Updated `useMarketSnapshot` to return `current_symbol` and `current_timeframe` from FXShinobi API. FXShinobiPage chart header shows "EURUSD · 15m" dynamically.
- **February 1, 2026:** Implemented broker adapter architecture with multi-broker support. Created broker registry supporting 6 brokers (OANDA, TastyFX, IG, Pepperstone, Forex.com, IC Markets). Added ConnectBrokerModal with two tabs: "Connect Broker" for credential entry with dynamic forms per broker, and "Active Broker" for switching between configured brokers. Backend routes `/api/fxshinobi/brokers` and `/api/fxshinobi/set-active-broker` proxy to FXShinobi API. Broker section in FXShinobiPage now shows active broker name, connection status, and 5 metrics (Balance, Equity, Open P&L, P&L Today, Open Positions). Status endpoint updated with `broker_configured`, `active_broker_id`, `active_broker_name`, `equity` fields replacing deprecated `oanda_configured`. Created `useBrokerRegistry` hook with `setActiveBroker` function.
- **February 1, 2026:** Refactored broker status detection to use boolean fields. Changed `BrokerInfo` and `FXShinobiBrokerData` types from `status: 'connected' | 'not_configured' | 'error'` string to `configured: boolean` and `connected: boolean` fields. FXShinobiPage now determines connection status by checking `activeBroker?.configured && activeBroker?.connected` from the `status.brokers` array. Metrics grid sources data from `status.account` with fallbacks to flat fields. The `useBrokerRegistry` hook merges broker status from FXShinobi's `/status` endpoint into the broker registry. Broker shows "Connected" (green) when configured+connected, "Configured (Error)" (amber) when configured but not connected, and "Not Configured" (red) otherwise.