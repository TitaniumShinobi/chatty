...
## Recent Changes
- **February 8, 2026:** Replaced trash/delete icon with 3-dot settings icon (MoreVertical) on GPT conversation items in sidebar. Clicking the 3-dot icon navigates directly to the GPT's edit/settings page (`/app/ais/edit/{constructId}`). Icon hidden for system constructs (Zen, Lin). Added user-scoped `getAIByCallsign()` fallback to `GET /api/ais/:id` route so GPTs can be looked up by constructCallsign (e.g., `katana-001`) with proper user ID resolution via `resolveVVAULTUserId`.
- **February 8, 2026:** Fixed transcript path stripping bug in `vvault.js` — construct IDs like `katana-001` were being stripped to `katana` in file paths (`instances/katana/chatty/` instead of `instances/katana-001/chatty/`). Now uses full construct ID for primary path with backward-compatible ENOENT fallback to legacy stripped paths. Also hardened model resolver: added detection for Ollama-style model patterns (`phi3:latest`, `word:tag`) that were causing OpenRouter 400 errors, and bare model names without provider prefix now fall back to `meta-llama/llama-3.3-70b-instruct` instead of being sent as invalid model IDs.
