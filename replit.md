...
- **Memory Context Builder (Always-On):** `server/lib/memoryContextBuilder.js` centralizes prompt construction by loading identity + capsule + ChromaDB memories + anti-roleplay directives into every message. Replaces the old bare prompt.txt path. Post-response memory capture stores exchanges for future retrieval.
- **Anti-Roleplay Enforcement:** System prompt directives prevent asterisk narration, third-person self-reference, and memory fabrication. Constructs must ground responses in actual capsule data and memory context.
