...
## Recent Changes
- **February 3, 2026:** Implemented ChatGPT-style image/vision upload support for GPT seats. Added ImageAttachmentPreview component with single large view, carousel for 3+ images, and lightbox zoom. Images converted to base64 in Layout.tsx and passed through aiService → VVAULT API → OpenAI vision API. System automatically routes to gpt-4o when images attached. Added chatConfig.ts with upload limits (10 images, 5 docs, 10MB max). Added error handling when OpenAI unavailable for vision requests.
