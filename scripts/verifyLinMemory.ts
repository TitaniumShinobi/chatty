...
const TEST_DB_PATH = './verify_lin_memory.db';
const LIN_ID = 'lin-001';
const USER_ID = 'verify-user-01';

async function run() {
    console.log('🧪 Starting Short-Term Memory Verification for Lin...');
            ...
            if (id === LIN_ID) {
                return {
                    config: {
                        id: LIN_ID,
                        name: 'Lin',
                        description: 'Lin Identity',
                        instructions: 'You are Lin.',
                        files: [],
                        actions: [],
                        isActive: true,
                        ...
                        userId: 'system',
                        testCapsule: { data: { metadata: { instance_name: 'Lin' }, traits: [], personality: {}, memory: {} } }
                    },
                    context: '',
                    memory: new Map(),
    ...
    // Ensure Lin is loaded
    console.log(`🔄 Loading GPT: ${LIN_ID}...`);
    try {
        const loaded = await runtimeService.loadGPT(LIN_ID);
        if (!loaded) console.error('Warning: loadGPT returned null');
    } catch (e) { console.error('Error loading GPT:', e); }

    // 3. Inject message
    const secretCode = `SIGMA-${Math.floor(Math.random() * 1000)}`;
    const secretFact = `My secret protocol is ${secretCode}`;

    console.log(`\n🗣️  Injecting User Message: "${secretFact}"`);
    await memoryStore.persistMessage(USER_ID, LIN_ID, secretFact, 'user');
    await memoryStore.persistMessage(USER_ID, LIN_ID, "Acknowledged.", 'assistant');

    // 4. Inject filler
    console.log('⏳ Injecting 5 filler turns...');
    for (let i = 1; i <= 5; i++) {
        await memoryStore.persistMessage(USER_ID, LIN_ID, `Filler ${i}`, 'user');
        await memoryStore.persistMessage(USER_ID, LIN_ID, `Response ${i}`, 'assistant');
    }

    // 5. Explicitly Verify Memory Store Retrieval FIRST
    console.log('\n🔍 Verifying Message Persistence in SQLite...');
    const history = await memoryStore.retrieveHistory(USER_ID, LIN_ID, 20);
    const foundInHistory = history.some(m => m.content.includes(secretCode));

    if (foundInHistory) {
        console.log('✅ SUCCESS: Secret protocol found in persistent memory retrieval.');
    } else {
        console.error('❌ FAILURE: Secret protocol NOT found in persistent memory.');
    ...
    try {
        await runtimeService.processMessage(LIN_ID, "What is my secret protocol?", USER_ID);
