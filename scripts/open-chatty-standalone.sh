...
CHATTY_STABILITY_SECONDS="${CHATTY_STABILITY_SECONDS:-3}"
...
if ! is_frontend_listening || ! is_frontend_reachable || ! is_backend_ready || ! bridge_ready || ! ollama_ready; then
  (
    cd "$CHATTY_REPO"
    nohup bash scripts/with-pinned-node.sh bash scripts/keep-running.sh >"$CHATTY_LAUNCH_LOG" 2>&1 </dev/null &
...
for ((i = 0; i < CHATTY_WAIT_SECONDS; i += 1)); do
  if is_frontend_listening && is_frontend_reachable && is_backend_ready && bridge_ready && ollama_ready; then
    ...
    if (( stable_seconds >= CHATTY_STABILITY_SECONDS )); then
      ready=1
      break
    fi
  else
