...
**Quick check if you still see the login screen after Google:**  
1. Google Cloud Console → Your OAuth 2.0 Client → **Authorized redirect URIs** must include exactly: `http://localhost:5173/api/auth/google/callback`.  
2. Backend must be running (e.g. port 5050; Vite proxies `/api` to it).  
3. Restart backend after any `.env` change, then try "Continue with Google" again.
