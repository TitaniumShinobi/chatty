...
> ARCHIVE SUPERSESSION WARNING: This archived analysis preserves old integration history. It is provenance only. Current VVAULT authority is the cloud/VVAULT-owned database with write/readback proof. Supabase, ChromaDB, local filesystem, and `instances/.../chatty` references below must not guide new transcript/continuity/sync work unless a current live authority document explicitly reactivates them. See `/Users/devonwoodson/Documents/GitHub/chatty/docs/standards/vvault-authority-contract.md`.
