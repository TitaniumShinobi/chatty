...
> ARCHIVE SUPERSESSION WARNING: This archived prompt contains old local/file-creation instructions. It is provenance only. Current VVAULT authority is the cloud/VVAULT-owned database with write/readback proof. Do not execute old instructions that create local transcript/archive/sync folders or treat `instances/.../chatty` paths as continuity authority. See `/Users/devonwoodson/Documents/GitHub/chatty/docs/standards/vvault-authority-contract.md`.
