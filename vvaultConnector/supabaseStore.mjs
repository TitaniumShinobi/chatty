...
function normalizeUserLookupContext(userEmailOrId) {
  if (userEmailOrId && typeof userEmailOrId === 'object' && !Array.isArray(userEmailOrId)) {
    ...
    const explicitSupabaseUserId = userEmailOrId.supabaseUserId || userEmailOrId.supabase_user_id || null;
    const supabaseUserId = UUID_RE.test(String(explicitSupabaseUserId || '').trim())
      ? String(explicitSupabaseUserId).trim()
      : null;
  ...
  return {
    ...
    userEmail: looksLikeEmail(normalizedValue) ? normalizedValue : null,
    supabaseUserId: UUID_RE.test(String(normalizedValue || '')) ? normalizedValue : null,
    primaryLookupId: normalizedValue,
  };
}
...
async function resolveSupabaseUserId(emailOrId) {
  ...
  return resolveSupabaseUserIdFromEmailOrId(emailOrId);
}
...
function deriveConstructIdFromFilename(filename = '') {
  const chatWithMatch = String(filename || '').match(/chat_with_([^/.]+)\.md$/i);
  if (chatWithMatch?.[1]) return chatWithMatch[1];
  const instanceMatch = String(filename || '').match(/^instances\/([^/]+)\//i);
  if (instanceMatch?.[1]) return instanceMatch[1];
  return null;
}

function toConstructTitle(constructId) {
  if (!constructId) return 'Conversation';
  const base = String(constructId).replace(/-\d+$/, '');
  return base ? base.charAt(0).toUpperCase() + base.slice(1) : 'Conversation';
}

function toConversationIndexRecord(row = {}) {
  const metadata = parseMetadata(row.metadata);
  const constructId = metadata.constructId || row.construct_id || deriveConstructIdFromFilename(row.filename) || null;
  const sessionId = metadata.sessionId || (constructId ? `${constructId}_chat_with_${constructId}` : row.filename || row.id);
  const metadataMessages = normalizeConversationMessages(
    sessionId,
    Array.isArray(metadata.messages) ? metadata.messages : [],
  );
  const messages = metadataMessages.slice(-5).map((m) => ({
    id: m.id,
    role: m.role || 'assistant',
    content: m.content || m.text || '',
    timestamp: m.timestamp || m.createdAt || resolveVaultFileUpdatedAt(row),
    isDateHeader: Boolean(m.isDateHeader),
  }));
  const updatedAt = resolveVaultFileUpdatedAt(row);
  const messageCount = Number.isFinite(Number(metadata.messageCount))
    ? Number(metadata.messageCount)
    : metadataMessages.length;
  const lastMessageAt = metadata.lastMessageAt || metadataMessages.slice(-1)[0]?.timestamp || updatedAt;
  return {
    id: sessionId,
    title: metadata.title || metadata.constructName || toConstructTitle(constructId),
    constructId,
    updatedAt,
    lastMessageAt,
    messageCount,
...
const VVAULT_SUPABASE_INDEX_QUERY_TIMEOUT_MS = Number(process.env.VVAULT_SUPABASE_INDEX_QUERY_TIMEOUT_MS || 3000);
...
async function withIndexQueryTimeout(promise, label) {
  const timeoutMs = normalizePositiveTimeout(VVAULT_SUPABASE_INDEX_QUERY_TIMEOUT_MS, 3000);
  let timeoutId = null;
  try {
    ...
    return await Promise.race([
      Promise.resolve(promise)
        .then((result) => ({ status: 'ok', result, error: null }))
        .catch((error) => ({ status: 'error', result: null, error })),
      timeoutPromise,
    ]);
  } finally {
...
async function runConversationIndexQuery({ supabase, supabaseUserId, constructId = null, fileType = null, filenameLike = null }) {
  const colsWithUpdated = 'id,user_id,filename,construct_id,created_at,updated_at,metadata,file_type';
  const colsWithoutUpdated = 'id,user_id,filename,construct_id,created_at,metadata,file_type';
  const execute = async (columns) => {
    let query = supabase
      .from('vault_files')
      .select(columns)
      .eq('user_id', supabaseUserId);
    if (constructId) query = query.eq('construct_id', constructId);
    if (fileType) query = query.eq('file_type', fileType);
    if (filenameLike) query = query.like('filename', filenameLike);
    return query.order('created_at', { ascending: false });
  };

  const queryLabel = fileType ? `file_type=${fileType}` : `filenameLike=${filenameLike || 'none'}`;

  let data = [];
  let error = null;
  let initialOutcome = await withIndexQueryTimeout(
    execute(colsWithUpdated),
    `Supabase index query (${queryLabel})`
  );
  if (initialOutcome.status === 'ok' && initialOutcome.result) {
    data = initialOutcome.result.data || [];
    error = initialOutcome.result.error || null;
  } else {
    error = initialOutcome.error || new Error(`Supabase index query failed (${queryLabel})`);
  }

  if (error && /updated_at/i.test(error.message || '')) {
    const fallbackOutcome = await withIndexQueryTimeout(
      execute(colsWithoutUpdated),
      `Supabase index query fallback (${queryLabel})`
    );
    if (fallbackOutcome.status === 'ok' && fallbackOutcome.result) {
      data = fallbackOutcome.result.data || [];
      error = fallbackOutcome.result.error || null;
    } else {
      data = [];
      error = fallbackOutcome.error || error;
    }
  }
  if (error) {
    return { rows: [], error };
...
async function readConversationIndexFromSupabase(userEmailOrId, constructId = null) {
  ...
  const [typed, chatty, hydro, legacy] = await Promise.all([
    runConversationIndexQuery({ supabase, supabaseUserId, constructId, fileType: 'conversation' }),
    runConversationIndexQuery({ supabase, supabaseUserId, constructId, filenameLike: 'instances/%/chatty/%' }),
    runConversationIndexQuery({ supabase, supabaseUserId, constructId, filenameLike: 'instances/hydro-001/code/%_hydro_chat.md' }),
    runConversationIndexQuery({ supabase, supabaseUserId, constructId, filenameLike: 'chat_with_%.md' }),
  ]);

  if (typed.error) console.warn(`⚠️ [SupabaseStore] Index query (typed) failed: ${typed.error.message}`);
  if (chatty.error) console.warn(`⚠️ [SupabaseStore] Index query (chatty path) failed: ${chatty.error.message}`);
  if (hydro.error) console.warn(`⚠️ [SupabaseStore] Index query (hydro path) failed: ${hydro.error.message}`);
  if (legacy.error) console.warn(`⚠️ [SupabaseStore] Index query (legacy path) failed: ${legacy.error.message}`);

  const mergedRows = [];
  const seenRowKeys = new Set();
  for (const row of [...typed.rows, ...chatty.rows, ...hydro.rows, ...legacy.rows]) {
  ...
  for (const row of mergedRows) {
    const record = toConversationIndexRecord(row);
    if (!record.id) continue;
    const current = bySession.get(record.id);
    if (!current) {
      bySession.set(record.id, record);
      continue;
    }
...
async function writeConversationToSupabase(params) {
  ...
  try {
    ...
    if (!isConversationCreated && (hasContent || hasAttachments)) {
      ...
      const duplicateExists = messages.some((message) => !message.isDateHeader && messageFingerprint(message) === messageFingerprint(newMessage));
      if (!duplicateExists) {
        messages.push(newMessage);
      } else {
        console.log(`⏭️ [SupabaseStore] Skipping duplicate conversation message for ${target.sessionId}`);
      }
    }
