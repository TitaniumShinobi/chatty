import {
  ...
  getCanonicalConstructIdFromThreadId,
  getDisplayTitleForConstructId,
  isGPTConstruct,
  routeIdForThread,
  normalizeZenThreadId,
  isZenThreadIdentifier,
...
describe('threadUtils', () => {
  describe('deduplicateThreadsById', () => {
    ...
    it('does not append stale supplementary messages into the canonical Zen thread', () => {
      const threads: Thread[] = [
        {
          ...
          messages: [
            { role: 'user', text: 'canonical Tuesday prompt' },
            { role: 'assistant', text: 'canonical Tuesday response' },
          ],
          constructId: 'zen-001',
        },
        {
          ...
          messages: [
            { role: 'user', text: 'canonical Tuesday prompt' },
            { role: 'assistant', text: 'canonical Tuesday response' },
            { role: 'user', text: 'stale browser-only Thursday prompt' },
          ],
          constructId: 'zen-001',
        },
      ];
      ...
      expect(result[0].hydrationComplete).toBe(true);
      expect(result[0].messages).toHaveLength(2);
      expect(result[0].messages.map((message) => message.text)).not.toContain(
        'stale browser-only Thursday prompt',
      );
    });
    ...
    it('should fall back to the other title when the canonical title is filename-like', () => {
      const threads: Thread[] = [
        {
          id: 'thread-1',
  ...
  describe('canonical route IDs', () => {
    it('extracts construct IDs from canonical conversation routes', () => {
      expect(getCanonicalConstructIdFromThreadId('nova-001_chat_with_nova-001')).toBe('nova-001');
      expect(getCanonicalConstructIdFromThreadId('gpt-example-001_chat_with_gpt-example-001')).toBe('gpt-example-001');
      expect(getCanonicalConstructIdFromThreadId('nova-001_chat_with_sera-001')).toBeNull();
      expect(getCanonicalConstructIdFromThreadId('session_123')).toBeNull();
    });

    it('derives readable titles from construct IDs', () => {
      expect(getDisplayTitleForConstructId('nova-001')).toBe('Nova');
      expect(getDisplayTitleForConstructId('gpt-example-001')).toBe('Gpt Example');
      expect(getDisplayTitleForConstructId('')).toBe('Conversation');
    });
  });
  ...
  describe('isGPTConstruct', () => {
    ...
    it('should return false for Val constructs', () => {
      expect(isGPTConstruct('val-001')).toBe(false);
      expect(isGPTConstruct('val')).toBe(false);
      expect(isGPTConstruct('VAL-001')).toBe(false);
    });
  ...
  describe('routeIdForThread', () => {
    ...
    it('should NOT route Val thread to GPT canonical format', () => {
      const threads: Thread[] = [
        { id: 'session_123_abc', title: 'Val', messages: [], constructId: 'val-001' },
      ];
  ...
  describe('isZenThreadIdentifier', () => {
    it('recognizes canonical and legacy Zen route ids only', () => {
      expect(isZenThreadIdentifier('zen-001_chat_with_zen-001')).toBe(true);
      expect(isZenThreadIdentifier('zen_123456789')).toBe(true);
      expect(isZenThreadIdentifier('zen-001_preview')).toBe(true);
      expect(isZenThreadIdentifier('nova-001_chat_with_nova-001')).toBe(false);
      expect(isZenThreadIdentifier('katana-001_chat_with_katana-001')).toBe(false);
      expect(isZenThreadIdentifier(null)).toBe(false);
    });
  });
