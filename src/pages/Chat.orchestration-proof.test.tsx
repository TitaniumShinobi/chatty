...
describe("Chat orchestration proof surface", () => {
  ...
  it.each([
    ...
    ["/app/chat/sera-001_chat_with_sera-001", /message sera/i],
    ["/app/chat/katana-001_chat_with_katana-001", /message katana/i],
  ])(
  ...
  it.each([
    ["Nova", "nova-001", /message nova/i],
    ["Sera", "sera-001", /message sera/i],
    ["Katana", "katana-001", /message katana/i],
  ])(
    "enables the %s composer only after exact full VVAULT hydration is ready",
    async (displayName, constructId, placeholderPattern) => {
    const sendMessage = jest.fn();
    const threadId = `${constructId}_chat_with_${constructId}`;
    renderChatWithContext({
      threads: [
        {
          id: threadId,
          ...
          constructId,
          isIndexHydrated: false,
          messages: [{ id: `${constructId}-1`, role: "assistant", text: `${displayName} is here.`, ts: Date.now() }],
        },
      ],
      sendMessage,
      activeThreadHydration: {
    ...
    fireEvent.change(textarea, { target: { value: "hydration is real" } });
  ...
  it("enables an empty Address Book thread after full VVAULT hydration is confirmed", async () => {
    const sendMessage = jest.fn();
    const threadId = "sera-001_chat_with_sera-001";
    ...
    const textarea = screen.getByPlaceholderText(/message sera/i);
    const sendButton = screen.getByLabelText("Send message");
    expect(textarea).not.toBeDisabled();
    expect(sendButton).toBeDisabled();

    fireEvent.change(textarea, { target: { value: "first turn after empty VVAULT create" } });
    expect(sendButton).not.toBeDisabled();
    fireEvent.click(sendButton);
    ...
    await waitFor(() => {
      expect(sendMessage).toHaveBeenCalledWith(
        threadId,
        "first turn after empty VVAULT create",
        [],
        undefined,
        undefined,
      );
  ...
  describe("Address Book contact send path", () => {
    function renderAddressBookContactChat(
      overrides: Record<string, unknown> = {},
    ) {
      return renderChatWithContext(
        {
          threads: [
            {
        ...
        "/app/chat/nova-001_chat_with_nova-001",
      );
    }

    it("enables the composer on a fresh empty canonical thread", () => {
      renderAddressBookContactChat();

      const textarea = screen.getByPlaceholderText(/message nova/i);
      expect(textarea).toBeEnabled();
    });

    it("forwards send to the Layout seam with the correct canonical threadId", async () => {
      const { outletContext } = renderAddressBookContactChat();

      const textarea = screen.getByPlaceholderText(/message nova/i);
      fireEvent.change(textarea, { target: { value: "hello Nova" } });
      fireEvent.keyDown(textarea, { key: "Enter", code: "Enter" });
    ...
    it("never shows raw transcript or VVAULT dead-end copy", () => {
      renderAddressBookContactChat();

      expectMissingCanonicalRouteTextAbsent();
      expectForbiddenChatDeadEndCopyAbsent();
      expect(screen.getByPlaceholderText(/message nova/i)).toBeInTheDocument();
      expect(
        screen.queryByText(/nova transcript/i),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(/VVAULT readback/i),
      ).not.toBeInTheDocument();
    });

    it("renders a sent user message in the normal chat shell", async () => {
      const { outletContext } = renderAddressBookContactChat();
      ...
      await waitFor(() => {
        expect(outletContext.sendMessage).toHaveBeenCalled();
      });

      // Shell elements must remain visible — no raw transcript
      expect(
        screen.getByPlaceholderText(/message nova/i),
      ).toBeInTheDocument();
      expect(screen.getByTestId("chat-message-scroller")).toBeInTheDocument();
    });
  });
