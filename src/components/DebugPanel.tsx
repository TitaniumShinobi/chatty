  ...
  return (
    <div className="fixed bottom-4 right-4 w-80 bg-app-orange-800 border border-app-orange-600 rounded-lg p-4 shadow-lg z-50">
      ...
      <div className="space-y-2 text-xs">
        <div>
          <span className="text-app-orange-400">Topic:</span>
          <span className="text-white ml-2">{context.topic}</span>
        </div>
        
        <div>
          <span className="text-app-orange-400">Mood:</span>
          <span className="text-white ml-2">{context.mood}</span>
        </div>
        
        <div>
          <span className="text-app-orange-400">User Role:</span>
          <span className="text-white ml-2">{context.userRole || 'none'}</span>
        </div>
        
        <div>
          <span className="text-app-orange-400">Current Intent:</span>
          <span className="text-white ml-2">{context.currentIntent}</span>
        </div>
        
        <div>
          <span className="text-app-orange-400">Previous Intents:</span>
          <div className="text-white ml-2">
            {context.previousIntents.slice(-3).map((intent: string, index: number) => (
              <span key={index} className="inline-block bg-app-orange-700 px-1 rounded mr-1 mb-1">
                {intent}
              </span>
            ))}
          </div>
        ...
        <div>
          <span className="text-app-orange-400">Message History:</span>
          <div className="text-white ml-2 max-h-20 overflow-y-auto">
            {context.conversationHistory.slice(-3).map((msg: string, index: number) => (
              <div key={index} className="text-xs text-app-orange-300 truncate">
                {msg}
              </div>
            ))}
          </div>
        ...
        <div className="pt-2 border-t border-app-orange-600">
          <button
            onClick={() => {
