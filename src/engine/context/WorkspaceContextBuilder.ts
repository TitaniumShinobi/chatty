...
export class WorkspaceContextBuilder {
  ...
  private async loadVVAULTTranscriptMetadata(
    ...
    try {
      const conversations =
        await this.conversationManager.loadAllConversations(userId);
      ...
      console.warn(
        "[WorkspaceContextBuilder] Failed to load VVAULT transcripts:",
        error,
      );
      return [];
    }
  ...
  private async loadMemoryLedger(
    ...
    try {
      ...
      const { MemoryLedger } = await import("../../lib/memoryLedger").catch(
        () => ({ MemoryLedger: null }),
      );
      ...
      try {
        const memories =
          await this.conversationManager.loadMemoriesForConstruct(
            userId,
        ...
        for (const memory of memories) {
          if (memory.context && typeof memory.context === "string") {
            ...
            if (
              /\b(?:claim|vow|greet|name|relationship)\b/i.test(memory.context)
            ) {
        ...
        console.warn(
          "[WorkspaceContextBuilder] Failed to load relationship anchors:",
          error,
        );
      }
      ...
      console.warn(
        "[WorkspaceContextBuilder] Failed to load memory ledger:",
        error,
      );
      return {
        continuityHooks: [],
