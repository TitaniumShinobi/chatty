...
export class PersonaLockdown {
     ...
     */
    enforcePersona(userMessage, llmResponse) {
     ...
     */
    checkSignatureResponses(userMessage) {
     ...
     */
    correctDrift(userMessage, originalResponse, drift) {
     ...
     */
    enforceBrevity(response) {
        if (!this.config.maxResponseLength)
            return response;
        const words = response.split(/\s+/);
        if (words.length <= this.config.maxResponseLength)
            return response;
 ...
 */
export function createKatanaLockdown() {
    const config = {
        constructId: 'gpt',
        callsign: 'katana-001',
        signatureResponses: [
 ...
 */
export function createPersonaLockdown(constructId, callsign, customConfig) {
