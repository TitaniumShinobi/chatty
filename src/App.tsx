...
export default function App() {
  ...
  useEffect(() => {
    const loadTurnstile = () => {
      ...
      if (!turnstileSiteKey) {
        console.error(
          "Turnstile site key is not configured. Set VITE_TURNSTILE_SITE_KEY in your environment.",
        );
        setTurnstileError(
          "Human verification is temporarily unavailable. Please contact support.",
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      console.warn(
        "⚠️ [App.tsx] Auth effect timeout - forcing isLoading to false",
      );
      setIsLoading(false);
    }, 5000);
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            console.warn(
              "⚠️ [App.tsx] Backend not responding, continuing anyway",
            );
          }
        }
        ...
        console.error(
          "❌ [App.tsx] Unexpected error in auth effect:",
          outerError,
        );
      } finally {
  ...
  async function handleEmailAuth(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "signup") {
        if (!turnstileSiteKey) {
          setAuthError(
            "Human verification configuration is missing. Please contact support.",
          );
          return;
        }
  ...
  const handleOAuthLogin = (provider: string, loginFunction: () => void) => {
    if (mode === "signup") {
      ...
      if (!acceptTerms) {
        setAuthError(
          "Please accept the Chatty Terms of Service before continuing.",
        );
        return;
      }
      if (!acceptVVAULTTerms) {
        setAuthError(
          "Please accept the VVAULT Terms of Service before continuing.",
        );
        return;
      }
  ...
  const AuthButtons = () => {
    ...
    const isOAuthDisabled =
      mode === "signup" &&
      (!acceptTerms || !acceptVVAULTTerms || !turnstileToken);
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        {isOAuthDisabled && (
          <div
            className="mt-3 text-xs text-center opacity-70"
        ...
        >
          ...
          {signupSuccess ? (
            ...
            <>
                    ...
                    {confirmPassword && password && (
                      <div className="mt-1 text-xs">
                        {password === confirmPassword ? (
                          <span
                            style={{
                              color: "var(--chatty-status-success, #22c55e)",
                            }}
                          >
                            ✓ Passwords match
                          </span>
                          ...
                          <span
                            style={{
                              color: "var(--chatty-status-error, #dc2626)",
                            }}
                          >
              ...
              <div
                className="mt-6 text-center opacity-85"
