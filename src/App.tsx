...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // No cached session, wait for backend and check API
        const { waitForBackendReady } = await import('./lib/backendReady');
        try {
          await waitForBackendReady(10, (attempt) => {
            if (attempt === 1) {
              console.log('⏳ [App.tsx] Waiting for backend to be ready...');
            }
          });
          console.log('✅ [App.tsx] Backend ready');
        } catch (error) {
          console.warn('⚠️ [App.tsx] Backend readiness check failed, continuing anyway:', error);
          // Continue anyway - fetchMe will handle retries
        }
        ...
        if (me) {
          ...
          try {
            ...
            if (migrated) {
              console.log('🎯 [App.tsx] Surgical migration completed - symbolic scoping breach recovered');
            }
          } catch (migrationError) {
            console.error('❌ [App.tsx] Migration failed:', migrationError);
  ...
  const SignupSuccess = () => (
    <div className="text-center py-8">
      ...
      <button
        ...
        className="w-80 py-2 rounded-full border font-semibold transition-colors"
        style={{ 
  ...
  console.log('🔍 [App.tsx] Render - isLoading:', isLoading, 'user:', user ? `${user.email} (${user.sub || user.id || 'no-id'})` : 'null');
  ...
  return (
    <>
      ...
      <div className="w-full max-w-md rounded-xl p-6 shadow-lg" style={{ backgroundColor: 'var(--chatty-bg-secondary)' }}>
        ...
        {signupSuccess ? (
          ...
          <>
            ...
            {/* Manual form - position depends on mode */}
          ...
          {authError && (
            <div className="border px-3 py-2 rounded-lg text-sm" style={{
              backgroundColor: 'var(--chatty-error-bg, #FEF2F2)',
              borderColor: 'var(--chatty-error-border, #FECACA)',
              color: 'var(--chatty-error-text, #DC2626)'
            }}>
              {authError}
            </div>
          )}
          ...
          <div className="flex justify-center">
            <button 
              ...
              className="w-80 py-2 rounded-full border font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              style={{ 
                backgroundColor: 'var(--chatty-button)', 
                color: 'var(--chatty-text-inverse)',
                borderColor: 'var(--chatty-button)'
            ...
            <div className="mt-6 text-center opacity-85">
              {mode === 'login' ? (
                <span>
                  New here?{' '}
                  <a 
